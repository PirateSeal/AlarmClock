...
{
    class ClockGatewayTests
    {
        public ClockGatewayTests()
        {
            Gateway = new ClockGateway();
        }
        private ClockGateway Gateway { get; set; }

        public void CheckClockData(Result<ClockData> clockData, string name, string guid, int password, IEnumerable<PresetData> presetList )
        {
        ...
        {
            var clockStatus = await Gateway.CreateClockData( "Test" );
